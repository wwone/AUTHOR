...
# following:
#
# article_print.fo     (FOP previously combined from AUTHOR output)
#                one way to combine the FOP files is the
#                following command:
#                cat book*.foprint > article_print.fo
#
...
# the following is AN EXAMPLE only, you will alter it to
# point to the installed FOP package on your computer
#
...
echo sh $SCRIPT -nocs -x -c $CONF article_print.fo article_print.pdf
sh $SCRIPT -nocs -x -c $CONF article_print.fo article_print.pdf
